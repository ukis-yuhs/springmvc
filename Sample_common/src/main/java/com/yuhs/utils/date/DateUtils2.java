...
public class DateUtils2 {
    ...
    public static String getTodayYyyyMmDd() {
        return parseString(getTime(), FMT_YYYY_MM_DD);
    }
    ...
    public static String getTodayYyyy() {
        return parseString(getTime(), FMT_YYYY);
    }
    ...
    public static String getStrTime() {
        return parseString(getTime(), FMT_DEFAULT);
    }
    ...
    public static String getSimpleTime(Date time) {
        long ssDiff = getDateDiff(time, getTime(), 0);
        ...
        long mmDiff = getDateDiff(time, getTime(), 1);
        if (mmDiff < 60) {
            result.append(mmDiff).append("分钟前");
            return result.toString();
        }
        String timeStr = parseString(time);
        if (getTodayYyyyMmDd().equals(timeStr.substring(0, 10))) {
            result.append("今天 ").append(parseString(time, "HH:mm"));
            return result.toString();
        }
        if (getTodayYyyy().equals(timeStr.substring(0, 4))) {
            return parseString(time, "MM月dd日 HH:mm");
        }
        return parseString(time, "yyyy年MM月dd日");
    }
}
